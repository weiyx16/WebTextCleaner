...
MIN_LANG_PROB = 0.6     # MIN PROBILITY OF LANGUAGE DETECTION
MIN_LANG_PROB_LONG = 0.85 # MIN PROBILITY OF LANGUAGE DETECTION for longer text (i.e. document)
# we found for fastText, short terms like: "Table 2: prediction of ethylene diffusion in jars." earns only 0.65, and for non-english, like "Market researchtium totam rem aperiam eaque..." (~0.2) be fused with some normal sentence, it will get really high score. So we add stronger constrain on long sentences to detect bad sentences and loose on short one which fastText tends to give a relative low predict.
...
bad_tokens_utf16 = [b'\xfd\xff']
# --------------


def get_len(text, lang):
    return len(list(filter(lambda x: len(x) > 0, text.split(" ")))) if blank(lang) else len(text)

def get_character_len(text, lang):
    return sum([len(ele) for ele in text.split(" ")]) if blank(lang) else 4

def get_first_word(text, lang):
    return list(filter(lambda x: len(x) > 0, text.split(" ")))[0] if blank(lang) else text[0]

def get_word_list(text, lang):
    return list(filter(lambda x: len(x) > 0, text.split(" "))) if blank(lang) else list(text)
...
def language_detect(sentence, is_long_texts=False):
    """
        ...
        Rule-1
    """
    min_lang_prob = MIN_LANG_PROB if not is_long_texts else MIN_LANG_PROB_LONG
    lang = LanDetectModel.predict(sentence)
    is_en = False
    try:
        if lang[0][0].endswith('en') and lang[1][0] > min_lang_prob:
            # prob: lang[1][0]
            is_en = True
    except:
        pass
...
def document_filter(doc):
    ...
    is_en = language_detect(' '.join(doc), is_long_texts=True)
    if is_en:
        lang = 'en'
    else:
        return None, False
    ...
    for Ele in doc:
        ...
        if sent_length >= MIN_SENTENCE_LENGTH:
            ...
            if language_detect(Ele) and 'javascript' not in ele:
                # Rule-11: web code 
                if not any(w in ele for w in bad_tokens) and not any(w in ele.encode('utf-16') for w in bad_tokens_utf16):
    ...
    if len(filtered_doc) < MIN_SENTENCE_INPAGE:
        return None, False
...
def detect_and_clean(tmp_f, tgt_f):
    ...
    tgt_blocks = dict()
    for b in tqdm(blocks, miniters=3000, mininterval=60):
